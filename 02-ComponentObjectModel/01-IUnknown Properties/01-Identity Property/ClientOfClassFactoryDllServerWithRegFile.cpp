...
{	
	...
	case WM_CREATE:
		...
		//to prove Identity property
		if(gpIUnknownFromISum == gpIUnknownFromISubtract)
			MessageBox(NULL, TEXT("Both IUnknowns are from Same Component"), TEXT("Identity Property"), MB_OK);
		else
			MessageBox(NULL, TEXT("Both IUnknowns are NOT from Same Component"), TEXT("Identity Property"), MB_OK);
